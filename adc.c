...
void ADC_Init(void){
    ...
    ADCON1bits.PCFG = 0b1111; //ADC input channels: [AN0-AN7]


    //ADCON2 Register
    ADCON2bits.ACQT = 0b100; //ADC aquisition time: 4*ADC_CLK_PERIOD = 4*250ns = 1us
    ADCON2bits.ADCS = 0b100; //ADC conversion clock period: 4*ADC_CLK_PERIOD = 4*250ns = 1us
...
//==============================================================================
void ADC_Read(void){
    while(ADCON0bits.GO_DONE == 1); //Wait for ADC conversion to complete
    ADC_Result = ADRESH; //Read ADC result
}