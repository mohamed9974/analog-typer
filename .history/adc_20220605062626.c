...
void ADC_Init(void){
    ...
    //ADCON2 Register
    ADCON2bits.ACQT = 0b010; //ADC aquisition time: 4*ADC_CLK_PERIOD = 4*250ns = 1us
    ADCON2bits.ADCS = 0b010; //ADC conversion clock period: 4*ADC_CLK_PERIOD = 4*250ns = 1us
    ADCON2bits.ADRC = 0b0; //ADC conversion clock source: FOSC/2
    
    //ADCON0 Register
    ADCON0bits.CHS = 0b0000; //ADC input channel: AN0
    ADCON0bits.ADCS = 0b100; //ADC conversion clock period: 4*ADC_CLK_PERIOD = 4*250ns = 1us
    ADCON0bits.GO_DONE = 0b0; //ADC conversion start: Disabled
    ADCON0bits.ADON = 1; //ADC module enable
    ADCON0bits.PCFG = 0b1100; //ADC input channels: [AN0-AN7]
...
void ADC_Read(byte channel){
    ...
    ADCON0bits.GO_DONE = 1; //ADC conversion start: Enabled
    while(ADCON0bits.GO_DONE == 1); //Wait for conversion to complete
}