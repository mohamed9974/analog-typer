...
void cdm_State_init(){
  ...
  LcdPrint(customChar[customCharPos]);
  // Display the custom character position and count on the seven segment display
  UpdateSevenSeg(customCharCount, customCharPos);
}

void move_cursor_up(){
  //check if the cursor is at the top of the custom character array
  if((customCharPos < 10){
    //set the cursor position to the last position of the array
    customCharPos += 60;
  }
  else{
  ...
  //update the custom character position and count on the seven segment display
  UpdateSevenSeg(customCharCount, customCharPos);
}

void move_cursor_down(){
  //check if the cursor is at the bottom of the custom character array
  if((customCharPos > 60){
    //set the cursor position to the first position of the array
    customCharPos -= 60;
  }
  else{
