...
void confirm_selection(){
  //light up the correct LED
  leds_grid_update();
  //update the custom character array
  customChar[customCharPos] = customCharCount;
...
void leds_grid_update(){
  //light up the correct LED
  leds_grid[custom_Char_pos_seg[1]][custom_Char_pos_seg[0]] = 1;
  //clear the rest of the LEDs
  for (int i = 0; i < 7; i++){
    for (int j = 0; j < 5; j++){
      if (i != custom_Char_pos_seg[1] || j != custom_Char_pos_seg[0]){
        leds_grid[i][j] = 0;
      }
    }
  }
